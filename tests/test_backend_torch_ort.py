...
def test_torch_convert_onnx(m):
    ...
    torch_convert_onnx(model=model, output_path='/tmp/torch', inputs_pytorch=inputs_pytorch, verbose=False)
    ...
    assert {'input': {0: 'batch_size'}, 'output': {0: 'batch_size'}} == kwargs['dynamic_axes']
